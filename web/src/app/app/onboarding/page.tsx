  ...
  const handleComplete = async () => {
    ...
    try {
      await api.submitOnboarding({
        ...
        debate_frequency: data.debateFrequency,
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5 flex items-center justify-center p-4">
      ...
      >
        ...
        <Card>
          ...
          <CardContent>
            {error && (
              <div className="mb-4 p-3 rounded-lg border border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-950/30 text-red-600 dark:text-red-400 text-sm">
                {error}
              </div>
            )}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {frequencyOptions.map((option) => (
                      <motion.button
                        ...
                        whileHover={{ scale: 1.02 }}
                        whileTap={{ scale: 0.98 }}
                        onClick={() => updateData("debateFrequency", option.value)}
                        className={`p-4 rounded-lg border-2 text-center transition-all ${
                          data.debateFrequency === option.value
                            ? "border-primary bg-primary/5"
                            : "border-border hover:border-primary/50"
                        }`}
                      >
