...
import { api, PersonaInfo } from "@/lib/api-app";

export default function NewDebateClient() {
  ...
  const handleStartDebate = async () => {
    ...
    try {
      ...
      router.push(`/app/debate/${session.id}`);
    } catch (err) {
  ...
  const personaDescriptions: Record<string, { icon: React.ElementType; color: string; traits: string[] }> = {
    ...
    devil_advocate: {
      ...
      traits: ["Contrarian", "Testing", "Extreme"],
    },
  };
                ...
                {isLoading ? (
                  ...
                  <div className="space-y-3">
                    {personas.map((persona) => {
                      const info = personaDescriptions[persona.id] || {
                        icon: MessageSquare,
                        color: "text-gray-500",
                        traits: [],
                      };
