...
FALLACY_LABELS = [
    ...
    "appeal_to_authority", "bandwagon", "circular_reasoning", "red_herring", "no_fallacy"
]
...
class ArgumentStrengthScorer:
    ...
    def _calculate_evidence(self, text: str) -> float:
        evidence_indicators = [
            "study", "research", "evidence", "data", "statistics", "percent",
            "according to", "shown", "demonstrated", "proven", "found",
            "example", "instance", "case", "source", "report"
        ]
        
        text_lower = text.lower()
        indicator_count = sum(1 for indicator in evidence_indicators if indicator in text_lower)
    ...
    def _calculate_sentiment(self, text: str) -> float:
        ...
        positive_words = ['good', 'great', 'excellent', 'better', 'benefit', 'success', 'improve', 'effective']
        negative_words = ['bad', 'terrible', 'worse', 'harm', 'failure', 'decline', 'weak', 'wrong', 'problem']
    ...
    def _calculate_logical_markers(self, text: str) -> float:
        logical_markers = [
            ...
            "however", "although", "nevertheless", "moreover", "furthermore",
            "first", "second", "third", "finally", "suggests", "implies", "indicates"
        ]
