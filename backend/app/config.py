...
def get_database_url() -> str:
    if os.getenv("VERCEL_ENV"):
        prisma_url = os.getenv("VERCEL_DB_PRISMA_DATABASE_URL") or os.getenv(
            "VERCEL_DB_PRISMA_POSTGRES_URL"
        )
        if prisma_url:
            if prisma_url.startswith("postgres://"):
                prisma_url = prisma_url.replace(
    ...
    def parse_cors(self):
        ...
        if cors_env.startswith("["):
            try:
                ...
                self.CORS_ORIGINS = [
                    c.strip() for c in cors_env.split(",") if c.strip()
                ]
        else:
